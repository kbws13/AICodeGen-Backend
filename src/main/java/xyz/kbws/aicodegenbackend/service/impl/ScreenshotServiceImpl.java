...
import xyz.kbws.aicodegenbackend.manager.CosManager;
import xyz.kbws.aicodegenbackend.service.ScreenshotService;
import xyz.kbws.aicodegenbackend.utils.WebScreenshotUtil;
...
public class ScreenshotServiceImpl implements ScreenshotService {
    ...
    public String generateAndUploadScreenshot(String webUrl) {
        // 参数校验
        ThrowUtil.throwIf(StrUtil.isBlank(webUrl), ErrorCode.PARAMS_ERROR, "截图的网址不能为空");
        log.info("开始生成网页截图，URL：{}", webUrl);
        // 本地截图
        String localScreenshotPath = WebScreenshotUtil.saveWebPageScreenshot(webUrl);
        ThrowUtil.throwIf(StrUtil.isBlank(localScreenshotPath), ErrorCode.OPERATION_ERROR, "生成网页截图失败");
        // 上传图片到 COS
        try {
            String cosUrl = uploadScreenshotToCos(localScreenshotPath);
            ThrowUtil.throwIf(StrUtil.isBlank(cosUrl), ErrorCode.OPERATION_ERROR, "上传截图到对象存储失败");
